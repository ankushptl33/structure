...
export const restMiddleware = store => next => async action => {
    ...
    try {
        ...
        let data = response.data || (await (await response).json()).data;
        ...
        // eventEmitter.emit(types.SUCCESS, {
        //   type: types.SUCCESS,
        //   payload: data,
        //   store,
        // });
        ...
        store.dispatch({ type: types.ERROR, payload: errorData });
        store.dispatch({ type: AppLoad, payload: false });
        handleAction({ type: types.ERROR, payload: errorData, store });
        // eventEmitter.emit(types.ERROR, {
        //   type: types.ERROR,
        //   payload: errorData,
        //   store,
        // });
...
//   return store.getState().app.isLoggedIn
//     ? store.getState().app.data.loginToken
//     : null;
// };
